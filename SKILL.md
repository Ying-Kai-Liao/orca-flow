...
6. **Watch the Orca card, not the terminal text.** Workers finish by setting their card to
   ...
   - **Context:** the `ctx` column (or `worktrees.py context`) shows each worker's estimated
     ...
     With `handoff.bin` configured, skip steps 1–2 and restart a flagged worker directly with
     `--continue`, because its jev-handoff working set is already current. The wrap-up line stays
     the fallback for repos without it.
